...
func getInterface() (device string, mac *net.HardwareAddr, gwip *net.IP, src *net.IP, err error) {
	...
	myIP = net.ParseIP("192.168.0.66").To4()
	newMac := getMAC(myIP)
...
func newScanner1(ip net.IP) (*scanner1, error) {
	s := &scanner1{
		dst: ip,
		opts: gopacket.SerializeOptions{
			FixLengths:       true,
...
func (s *scanner1) send(l ...gopacket.SerializableLayer) error {
