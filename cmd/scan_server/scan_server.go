...
func main() {
	f, err := os.Open("nmap-service-probes.txt")
	if err != nil {
		panic(err)
	}
	defer f.Close()
	...
	var count int
	var probe scan_server.Probe
	var probes []scan_server.Probe
	for scan.Scan() {
		count++
		...
		if strings.HasPrefix(line, "Probe ") {
			...
			probe = scan_server.Probe{
				...
				Rarity:       "",
				Fallback:     "",
				Matches:      []scan_server.Match{},
				Softmatches:  []scan_server.Softmatche{},
			}
			...
			softmatch := scan_server.Softmatche{
				Pattern:     regx,
				Name:        name,
				PatternFlag: regxFlag,
			}
